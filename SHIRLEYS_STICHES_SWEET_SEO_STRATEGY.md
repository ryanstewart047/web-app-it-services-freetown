# Shirley's Stiches & Sweet SEO Strategy

Page: `https://www.itservicesfreetown.com/shirleys`

## Primary Goal

Make Shirley's Stiches & Sweet discoverable for Freetown searches around bakery, pastries, celebration treats, custom fashion, tailoring, alterations, and event bundles.

## Keyword Targets

Primary local keywords:

- bakery in Freetown
- pastries in Freetown
- custom cakes Freetown
- small chops Freetown
- birthday treat boxes Freetown
- fashion designer Freetown
- tailoring in Freetown
- dress alterations Freetown
- custom outfits Freetown
- African wear Sierra Leone

Brand keywords:

- Shirley's Stiches & Sweet
- Shirley's Stiches and Sweet
- Shirley's Stitches & Sweet
- Shirley's Stitches and Sweet

## On-Page SEO Added

- Dedicated standalone page at `/shirleys`
- Unique title and meta description
- Canonical URL
- Open Graph and Twitter share metadata
- Dynamic branded share image at `/shirleys/opengraph-image`
- Local business structured data using `Bakery` and `ClothingStore`
- Sitemap entry for Google discovery
- Content sections for bakery, pastries, fashion, tailoring, bundles, and local Freetown service keywords

## Google Visibility Launch Steps

1. Add real product photos as soon as possible:
   - cupcakes, cakes, pastries, small chops
   - dresses, alterations, matching outfits
   - behind-the-scenes sewing or baking process

2. Create or update a Google Business Profile for Shirley's Stiches & Sweet:
   - category: Bakery
   - secondary category: Fashion designer or Clothing alteration service
   - service area: Freetown, Jui, Waterloo, Western Area
   - website URL: `https://www.itservicesfreetown.com/shirleys`

3. Post weekly short updates:
   - "Birthday treat box delivered in Freetown"
   - "Custom dress and pastry bundle for a family event"
   - "Fresh small chops available this weekend"
   - "Dress alteration slots open this week"

4. Ask happy customers for reviews using the brand name:
   - "Shirley's Stiches & Sweet made my birthday pastries..."
   - "Shirley adjusted my dress..."

5. Share the page link on social channels:
   - Facebook page posts
   - WhatsApp status
   - Instagram bio
   - local community groups where allowed

## Content Expansion Plan

Add these future pages only when there are real photos and examples:

- `/shirleys/custom-cakes-freetown`
- `/shirleys/pastries-small-chops-freetown`
- `/shirleys/fashion-tailoring-freetown`
- `/shirleys/birthday-treat-boxes-freetown`
- `/shirleys/dress-alterations-freetown`

Each page should show real work, order details, location terms, and customer questions. Avoid creating empty keyword pages.

## Contact Note

The current page uses the existing site WhatsApp number: `+232 33 399 391`. Replace it with Shirley's direct business number when available.

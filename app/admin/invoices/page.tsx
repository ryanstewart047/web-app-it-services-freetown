...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  draft:     { label: 'Draft',     color: 'bg-gray-500/20 text-gray-300 border-gray-500/40',     icon: <Edit3 className="w-3 h-3" /> },
  pending:   { label: 'Pending',   color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/40', icon: <Clock className="w-3 h-3" /> },
  part_paid: { label: 'Part Paid', color: 'bg-blue-500/20 text-blue-300 border-blue-500/40',     icon: <CreditCard className="w-3 h-3" /> },
  paid:      { label: 'Paid',      color: 'bg-green-500/20 text-green-300 border-green-500/40',  icon: <CheckCircle className="w-3 h-3" /> },
  overdue:   { label: 'Overdue',   color: 'bg-red-500/20 text-red-300 border-red-500/40',        icon: <AlertTriangle className="w-3 h-3" /> },
  cancelled: { label: 'Cancelled', color: 'bg-slate-500/20 text-slate-300 border-slate-500/40', icon: <XCircle className="w-3 h-3" /> },
};
...
export default function InvoicesAdminPage() {
        ...
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Segoe UI', Arial, sans-serif; color: #1a1a2e; background: white; padding: 40px; }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 text-white">
      ...
      {/* Idle Warning */}
      {showIdleWarning && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-red-600 text-white text-center py-3 animate-pulse text-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-400 via-indigo-400 to-violet-400 bg-clip-text text-transparent flex items-center gap-3">
              <FileText className="w-8 h-8 text-blue-400" />
              Invoice Generator
            </h1>
            <p className="text-slate-400 text-sm mt-1">Create, manage, and track professional invoices</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition-all border border-white/10"
            >
              <Plus className="w-4 h-4" /> New Invoice
            </button>
            <button
              onClick={() => setTab(tab === 'builder' ? 'history' : 'builder')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all border ${tab === 'history' ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/10 border-white/10 text-slate-300 hover:bg-white/20'}`}
            >
        ...
        {/* Auto-Save Status */}
        <div className="bg-blue-950/40 border border-blue-500/25 rounded-xl px-4 py-2.5 mb-6 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2.5">
            <span className="relative flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isSaving ? 'bg-amber-400' : 'bg-blue-400'}`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${isSaving ? 'bg-amber-500' : 'bg-blue-500'}`}></span>
            </span>
            <span className="text-blue-300 font-medium">
              {isSaving ? 'Saving draft...' : lastSaved ? `Draft auto-saved at ${lastSaved}` : 'Auto-Save enabled'}
            </span>
            {draftRestored && (
              <span className="bg-blue-500/20 text-blue-300 border border-blue-500/30 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Restored</span>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className={`space-y-5 ${previewMode ? 'hidden xl:block' : ''}`}>
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                <h2 className="text-sm font-bold text-slate-300 uppercase tracking-widest mb-4">Invoice Details</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={invoice.invoiceNumber} onChange={e => setInvoice(p => ({ ...p, invoiceNumber: e.target.value }))}
                      className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Status</label>
                    <select value={invoice.status} onChange={e => setInvoice(p => ({ ...p, status: e.target.value }))}
                      className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500">
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                <h2 className="text-sm font-bold text-slate-300 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <User className="w-4 h-4 text-blue-400" /> Bill To
                </h2>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={f.key} className={f.key === 'clientAddress' ? 'col-span-2' : ''}>
                      ...
                      <input value={(invoice as any)[f.key]} onChange={e => setInvoice(p => ({ ...p, [f.key]: e.target.value }))}
                        ...
                        className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-slate-600" />
                    </div>
                  ))}
                  <div className="col-span-2">
                    ...
                    <textarea value={invoice.clientAddress} onChange={e => setInvoice(p => ({ ...p, clientAddress: e.target.value }))}
                      ...
                      className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 resize-none placeholder-slate-600" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-bold text-slate-300 uppercase tracking-widest">Services / Items</h2>
                  <button onClick={addItem} className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-blue-600/30 hover:bg-blue-600/50 text-blue-300 rounded-lg transition-all border border-blue-500/30">
                    ...
                    <div key={item.id} className="grid grid-cols-12 gap-2 items-center group">
                      ...
                      <div className="col-span-2">
                        <input type="number" min="1" value={item.quantity} onChange={e => updateItem(item.id, 'quantity', parseFloat(e.target.value) || 1)}
                          className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 text-center" />
                      </div>
                      <div className="col-span-3">
                        <input type="number" min="0" step="0.01" value={item.unitPrice} onChange={e => updateItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0)}
                          className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 text-right" />
                      </div>
                ...
                <div className="mt-6 border-t border-white/10 pt-5 space-y-2.5">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <input type="number" min="0" max="100" step="0.5" value={invoice.taxRate} onChange={e => setInvoice(p => ({ ...p, taxRate: parseFloat(e.target.value) || 0 }))}
                      className="w-24 bg-slate-800 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm text-right focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {invoice.taxAmount > 0 && (
                    <div className="flex justify-between text-sm text-slate-400">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <input type="number" min="0" value={invoice.discountAmount} onChange={e => setInvoice(p => ({ ...p, discountAmount: parseFloat(e.target.value) || 0 }))}
                      className="w-32 bg-slate-800 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm text-right focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {invoice.discountAmount > 0 && (
                    <div className="flex justify-between text-sm text-slate-400">
                      <span>Discount</span>
                      <span className="font-mono text-green-400">- {fmt(invoice.discountAmount)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <input type="number" min="0" value={invoice.amountPaid} onChange={e => setInvoice(p => ({ ...p, amountPaid: parseFloat(e.target.value) || 0 }))}
                      className="w-36 bg-slate-800 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm text-right focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {invoice.amountPaid > 0 && (
                    <div className="flex justify-between text-sm text-slate-400">
                      <span>Amount Paid</span>
                      <span className="font-mono text-green-400">- {fmt(invoice.amountPaid)}</span>
                    </div>
                  )}
                  <div className="flex justify-between font-bold text-xl border-t-2 border-blue-500/50 pt-3">
                    <span className="text-white">Balance Due</span>
                    <span className={`font-mono ${invoice.balanceDue > 0 ? 'text-red-400' : 'text-green-400'}`}>{fmt(invoice.balanceDue)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 space-y-4">
                <div>
                  <label className="text-xs text-slate-400 mb-1 block uppercase tracking-widest font-bold">Notes</label>
                  <textarea value={invoice.notes} onChange={e => setInvoice(p => ({ ...p, notes: e.target.value }))}
                    placeholder="Additional notes, terms, warranty info..."
                    rows={3} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 resize-y placeholder-slate-600" />
                </div>
                <div>
                  <label className="text-xs text-slate-400 mb-1 block uppercase tracking-widest font-bold">Payment Instructions</label>
                  <textarea value={invoice.paymentInstructions} onChange={e => setInvoice(p => ({ ...p, paymentInstructions: e.target.value }))}
                    rows={6} className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-blue-500 resize-y font-mono text-xs" />
                </div>
              </div>
              ...
              <div className="flex gap-3 flex-wrap">
                <button onClick={saveInvoice} disabled={saving}
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 rounded-xl font-semibold transition-all">
                  {saving ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  {saving ? 'Saving...' : 'Save Invoice'}
                </button>
                <button onClick={() => setPreviewMode(!previewMode)}
                  className="flex items-center gap-2 px-5 py-3 bg-white/10 hover:bg-white/20 rounded-xl transition-all xl:hidden border border-white/10">
                  <Eye className="w-4 h-4" /> {previewMode ? 'Edit' : 'Preview'}
                </button>
                <button onClick={handlePrint}
                  className="flex items-center gap-2 px-5 py-3 bg-slate-700 hover:bg-slate-600 rounded-xl transition-all border border-white/10">
            ...
            <div className={`${!previewMode ? 'hidden xl:block' : ''}`}>
              <div className="sticky top-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Live Preview</h2>
                  <div className="flex gap-2">
                    <button onClick={handlePrint} className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg transition-all">
                ...
                {/* Invoice Document */}
                <div ref={printRef} className="bg-white rounded-2xl shadow-2xl text-gray-900 overflow-hidden" style={{ fontFamily: 'Segoe UI, Arial, sans-serif' }}>
                  ...
                  <div className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white px-8 py-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="text-blue-200 text-xs mt-1">Professional IT Services · Freetown, Sierra Leone</div>
                        <div className="text-blue-200 text-xs">+232 78 000 000 · info@itservicesfreetown.com</div>
                      </div>
                      <div className="text-right">
                        <div className="text-3xl font-black tracking-widest opacity-30">INVOICE</div>
                        ...
                        <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide
                          ...
                          : invoice.status === 'part_paid' ? 'bg-blue-300 text-blue-900'
                          : invoice.status === 'draft' ? 'bg-gray-300 text-gray-800'
                  ...
                  <div className="px-8 py-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6 text-xs">
                      {[
                        { label: 'Issue Date', value: invoice.invoiceDate },
                        { label: 'Payment Terms', value: invoice.paymentTerms },
                        { label: 'Due Date', value: invoice.dueDate },
                      ].map(d => (
                        <div key={d.label} className="bg-gray-50 rounded-lg p-3">
                          <div className="text-gray-400 uppercase tracking-widest text-[9px] mb-1">{d.label}</div>
                          <div className="font-semibold text-gray-800">{d.value}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-6">
                      <div className="bg-gray-50 rounded-xl p-4">
                        <div className="text-[9px] text-gray-400 uppercase tracking-widest font-bold mb-2">From</div>
                        <div className="font-bold text-gray-800">{BRAND_NAME}</div>
                        <div className="text-xs text-gray-500 mt-1">15 Siaka Stevens St, Freetown</div>
                        <div className="text-xs text-gray-500">Sierra Leone</div>
                      </div>
                      <div className="bg-blue-50 rounded-xl p-4">
                        <div className="text-[9px] text-blue-400 uppercase tracking-widest font-bold mb-2">Bill To</div>
                        <div className="font-bold text-gray-800">{invoice.clientName || <span className="text-gray-300">Client Name</span>}</div>
                        {invoice.clientCompany && <div className="text-xs text-gray-600">{invoice.clientCompany}</div>}
                        {invoice.clientEmail && <div className="text-xs text-gray-500">{invoice.clientEmail}</div>}
                        {invoice.clientPhone && <div className="text-xs text-gray-500">{invoice.clientPhone}</div>}
                        {invoice.clientAddress && <div className="text-xs text-gray-500 mt-1 whitespace-pre-line">{invoice.clientAddress}</div>}
                        {invoice.clientTaxId && <div className="text-xs text-gray-400 mt-1">TIN: {invoice.clientTaxId}</div>}
                      </div>
                    </div>
                    ...
                    <table className="w-full text-xs mb-6">
                      <thead>
                        <tr className="bg-blue-700 text-white">
                          <th className="text-left px-4 py-2.5 rounded-tl-lg font-semibold">#</th>
                          <th className="text-left px-4 py-2.5 font-semibold">Description</th>
                          <th className="text-center px-4 py-2.5 font-semibold">Qty</th>
                          <th className="text-right px-4 py-2.5 font-semibold">Unit Price</th>
                          <th className="text-right px-4 py-2.5 rounded-tr-lg font-semibold">Total</th>
                        </tr>
                      </thead>
                      <tbody>
                        {invoice.items.filter(i => i.description).map((item, idx) => (
                          <tr key={item.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            ...
                            <td className="px-4 py-2.5 text-right text-gray-600 font-mono">{fmt(item.unitPrice)}</td>
                            <td className="px-4 py-2.5 text-right text-gray-800 font-mono font-semibold">{fmt(item.total)}</td>
                          </tr>
                        ))}
                        {invoice.items.filter(i => i.description).length === 0 && (
                          <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-300 text-xs">No items added yet</td></tr>
                        )}
                      </tbody>
                    </table>
                    ...
                    <div className="flex justify-end mb-6">
                      <div className="w-72 space-y-1.5 text-xs">
                        <div className="flex justify-between text-gray-600"><span>Subtotal</span><span className="font-mono">{fmt(invoice.subtotal)}</span></div>
                        {invoice.taxAmount > 0 && <div className="flex justify-between text-gray-600"><span>Tax ({invoice.taxRate}%)</span><span className="font-mono">+ {fmt(invoice.taxAmount)}</span></div>}
                        {invoice.discountAmount > 0 && <div className="flex justify-between text-green-600"><span>Discount</span><span className="font-mono">- {fmt(invoice.discountAmount)}</span></div>}
                        <div className="flex justify-between font-bold text-sm border-t border-gray-200 pt-2 text-gray-900"><span>Total</span><span className="font-mono">{fmt(invoice.totalAmount)}</span></div>
                        {invoice.amountPaid > 0 && <div className="flex justify-between text-green-600 text-xs"><span>Amount Paid</span><span className="font-mono">- {fmt(invoice.amountPaid)}</span></div>}
                        <div className={`flex justify-between font-black text-base border-t-2 pt-2 ${invoice.balanceDue > 0 ? 'border-red-300 text-red-600' : 'border-green-300 text-green-600'}`}>
                    ...
                    {invoice.paymentInstructions && (
                      <div className="bg-blue-50 rounded-xl p-4 mb-4">
                        <div className="text-[9px] text-blue-500 uppercase tracking-widest font-bold mb-2">Payment Details</div>
                        <pre className="text-xs text-gray-700 whitespace-pre-wrap font-sans leading-relaxed">{invoice.paymentInstructions}</pre>
                      </div>
                    )}
                    ...
                    {invoice.notes && (
                      <div className="bg-yellow-50 rounded-xl p-4 mb-4 border border-yellow-100">
                        <div className="text-[9px] text-yellow-600 uppercase tracking-widest font-bold mb-1">Notes</div>
                    ...
                    <div className="border-t border-gray-100 pt-4 text-center text-[10px] text-gray-400">
                      Thank you for your business! · {BRAND_NAME} · Freetown, Sierra Leone
                    </div>
                  </div>
                </div>
              </div>
        ...
        {tab === 'history' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                { label: 'Total Invoiced', value: fmt(totalIssued), color: 'from-blue-600/20 to-blue-800/20 border-blue-500/30', text: 'text-blue-300' },
                { label: 'Collected', value: fmt(totalCollected), color: 'from-green-600/20 to-green-800/20 border-green-500/30', text: 'text-green-300' },
                { label: 'Balance Pending', value: fmt(totalPending), color: 'from-yellow-600/20 to-yellow-800/20 border-yellow-500/30', text: 'text-yellow-300' },
                { label: 'Paid Invoices', value: paidCount.toString(), color: 'from-emerald-600/20 to-emerald-800/20 border-emerald-500/30', text: 'text-emerald-300' },
                { label: 'Overdue', value: overdueCount.toString(), color: 'from-red-600/20 to-red-800/20 border-red-500/30', text: 'text-red-300' },
              ].map(c => (
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-wrap gap-3 items-center">
              <div className="relative flex-1 min-w-48">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search invoices..."
                  className="w-full pl-10 pr-3 py-2 bg-slate-800 border border-white/10 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                className="px-3 py-2 bg-slate-800 border border-white/10 rounded-lg text-white text-sm focus:ring-2 focus:ring-blue-500">
                <option value="all">All Statuses</option>
                {Object.entries(STATUS_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
              <button onClick={fetchInvoices} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm transition-all">
              ...
              <button onClick={() => { setInvoice(blankInvoice()); setTab('builder'); setPreviewMode(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-sm transition-all">
                      ...
                      {savedInvoices.map(inv => {
                        ...
                        return (
                          <tr key={inv.invoiceNumber} className="hover:bg-white/5 transition-colors group">
                            <td className="px-5 py-3.5 font-mono text-blue-400 text-xs font-bold">{inv.invoiceNumber}</td>
                            ...
                            <td className={`px-5 py-3.5 text-right font-mono text-xs font-bold hidden md:table-cell ${inv.balanceDue > 0 ? 'text-red-400' : 'text-green-400'}`}>
                              {fmt(inv.balanceDue)}
                            </td>

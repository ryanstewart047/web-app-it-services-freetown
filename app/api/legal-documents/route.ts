...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = {
      ...
      recipientSignerName: data.recipientSignerName || '',
      recipientSignerRole: data.recipientSignerRole || '',
      recipientSignDate: data.recipientSignDate || data.issueDate || new Date().toISOString().split('T')[0],
      includeClientSignature: data.includeClientSignature !== false,

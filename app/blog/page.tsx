...
At IT Services Freetown, we take your privacy seriously. Visit us at 37 Kissy Road or call +23233399391.`,
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hash) {
      ...
      if (post) {
        ...
        // Create OG image URL
        const ogImageUrl = new URL('/api/og-blog', window.location.origin)
        ogImageUrl.searchParams.set('title', post.title)
        ogImageUrl.searchParams.set('author', post.author)
        ogImageUrl.searchParams.set('date', formatDate(post.date))
  ...
  const handleShare = async (post: BlogPost) => {
    ...
    // Create a brief excerpt for sharing
    ...
    // Fallback: Copy link to clipboard with rich preview info
    try {
      const clipboardText = `${shareText}\n${shareUrl}`
      
      await navigator.clipboard.writeText(clipboardText)
      setCopiedPostId(post.id)
      toast.success('Link copied! Share preview will show post image and details.')
            ...
            posts.map((post, index) => (
              <div key={post.id} id={`post-${post.id}`}>
                ...
                <div className="p-8 md:p-10">
                  ...
                  {/* Title */}
                  <h2 className="text-3xl md:text-4xl font-bold mb-8 bg-gradient-to-r from-red-600 via-[#040e40] to-red-600 bg-clip-text text-transparent leading-tight group-hover:scale-[1.02] transition-transform duration-300">
                    {post.title}
                  </h2>

                  {/* Featured Image - Single main image right after title */}
                  {(post.media?.find(m => m.type === 'image') || post.image) && (
                    <div className="mb-8 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300">
                      <img 
                        src={post.media?.find(m => m.type === 'image')?.url || post.image} 
                        alt={post.media?.find(m => m.type === 'image')?.caption || post.title} 
                        className="w-full h-auto object-cover max-h-[500px]"
                      />
                      {post.media?.find(m => m.type === 'image')?.caption && (
                        <div className="bg-gradient-to-r from-red-50 to-gray-50 px-6 py-3">
                          <p className="text-sm text-gray-700 italic">{post.media.find(m => m.type === 'image')?.caption}</p>
                        </div>
                      )}
                    </div>
                  )}
